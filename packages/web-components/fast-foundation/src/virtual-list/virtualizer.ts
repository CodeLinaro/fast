...
export class Virtualizer {
     ...
     */
    public defaultItemSize = 50;

    /**
     * Viewport buffer to use if one is not specified
     */
    public defaultViewportBuffer = 100;
    ...
    protected requestPositionUpdates(): void {
        if (!this.containerElement || !this.viewportElement) {
            return;
        }
    ...
    private initializeResizeDetector(): void {
        ...
        ).ResizeObserver(this.resizeDetected.bind(this));
    ...
    private recalculateSizeMap(): void {
        ...
        let currentPosition: number =
            this.pendingSizemap[this.pendingSizemapChangeIndex].start;
    ...
    private updateVisibleItems(): void {
        ...
        if (this.virtualizationDisabled) {
            this.renderedItems.splice(0, this.renderedItems.length, ...this.sourceItems);
            this.updateVisibleItemSizes(0, this.renderedItems.length - 1);
            return;
        }
